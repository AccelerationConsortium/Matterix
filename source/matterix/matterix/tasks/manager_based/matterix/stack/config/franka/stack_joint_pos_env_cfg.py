...
class FrankaCubeStackEnvCfg(StackEnvCfg):
    def __post_init__(self):
        ...
        # Set Franka as robot
        self.scene.robot2 = FRANKA_PANDA_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot2")
        self.scene.robot2.init_state.pos = (1.0, 0, 0)
        self.scene.robot2.spawn.semantic_tags = [("class", "robot")]
        ...
        add_action_terms(self.actions, self.scene)
        add_event_terms(self.events, self.scene)
...
def add_action_terms(actions, scene):
...
def add_event_terms(events, scene):
    for asset_name, asset_cfg in scene.__dict__.items():
        if isinstance(asset_cfg, ArticulationCfg):
