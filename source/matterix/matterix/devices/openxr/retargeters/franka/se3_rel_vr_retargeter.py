...
class Se3RelVRRetargeter(RetargeterBase):
    """Retargets OpenXR motion controller data to end-effector commands using relative positioning.
    ...
    def retarget(self, data: dict) -> torch.Tensor:
        ...
        if len(controller_data) > DeviceBase.MotionControllerDataRowIndex.POSE.value:
            wrist = controller_data[DeviceBase.MotionControllerDataRowIndex.POSE.value]

        delta_wrist = self._calculate_delta_pose(wrist, self._previous_wrist)
        ee_command_np = self._retarget_rel(delta_wrist)
    ...
    def _retarget_rel(self, wrist: np.ndarray) -> np.ndarray:
...
@dataclass
class Se3RelVRRetargeterCfg(RetargeterCfg):
    ...
    retargeter_type: type[RetargeterBase] = Se3RelVRRetargeter
