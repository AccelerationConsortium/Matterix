...
class MoveToFrame(Action):
    def __init__(self, object : str, frame : str, asset = "robot", num_envs : int = 1, device: torch.device =  torch.device("cuda" if torch.cuda.is_available() else "cpu"), max_duration: int = 100):
        """
        Args:
            num_envs: Number of parallel environments
            device: Device to use for tensors
    ...
    def _compute_action_impl(self, env, env_ids: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        target_positions_w = env.unwrapped.scene[self.object].data.root_pos_w + torch.tensor(env.unwrapped.scene[self.object].cfg.frames[self.frame], device=self.device)
...
class MoveRelative(Action):
    def __init__(self, offset, asset = "robot", num_envs : int = 1, device: torch.device =  torch.device("cuda" if torch.cuda.is_available() else "cpu"), max_duration: int = 100):
        """
        Args:
            num_envs: Number of parallel environments
            device: Device to use for tensors
    ...
    def _compute_action_impl(self, env, env_ids: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        # Convert target positions from world frame to robot base frame
        if self.target_positions_w == None:
            self.target_positions_w = env.unwrapped.scene["ee_frame"].data.target_pos_w[:, 0, :].clone() + torch.tensor(self.offset, device=self.device)
